...
def main():
    ...
    def defineResponseCnt(respGen):
        if respGen == 0:
            print("How many responses would you like to generate?")
            respGen = input()
            respGen = int(respGen)
            print(respGen)
            return respGen

    respGen = defineResponseCnt(respGen)
    print(type(respGen))
    ...
    def surveyAnalysis():
        ...
        # Identify if the survey is a CX or EX survey
        surveyTitle = driver.find_element(By.ID, 'question-display-QID11').text
        if surveyTitle.__contains__('CX'):
            surveyType = 'CX'
            return surveyType
        
        #Fill in placeholders
        elif surveyTitle.__contains__(''):
            surveyType = 'CX'
            return surveyType
